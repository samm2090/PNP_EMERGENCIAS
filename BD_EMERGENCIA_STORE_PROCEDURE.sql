...
CREATE PROCEDURE USP_ENVIAR_EQUIPO
	@EME_ID INT,
	@EME_HORA_INICIO TIME(0)
AS
BEGIN TRAN
	BEGIN TRY
		UPDATE EMERGENCIA SET EME_HORA_INICIO = @EME_HORA_INICIO
		WHERE EME_ID = @EME_ID
	END TRY
	BEGIN CATCH
		ROLLBACK TRAN
...
CREATE PROCEDURE USP_EMERGENCIAS_ACTIVAS
AS
SELECT * FROM EMERGENCIA
WHERE EME_ID NOT IN (SELECT EME_ID FROM PARTE) 
GO
